...
class Referral(Base):
    ...
    referrer = relationship("User", foreign_keys=[referrer_user_id], back_populates="referrals_sent")
    referred = relationship("User", foreign_keys=[referred_user_id], back_populates="referral_received")

    __table_args__ = (
        UniqueConstraint("referred_user_id", name="unique_referral_referred_user"),
        UniqueConstraint("referrer_user_id", "referred_user_id", name="unique_referrer_referred_pair"),
    )
